...
class FrameworkDetector {
  ...
  /**
   * Detect specific frontend preset from package.json
   */
  ...
  /**
   * Read suggested env vars from .env.example files
   */
        ...
        for (const line of lines) {
          ...
          if (eqIndex > 0) {
            ...
            if (key && !suggested.some((s) => s.key === key)) {
              suggested.push({ key, value: value || '', isPublic: key.startsWith('VITE_') || key.startsWith('NEXT_PUBLIC_') || key.startsWith('REACT_APP_') });
            }
          }
        }
  ...
  /**
   * Suggest deployable root directories in a monorepo
   */
   ...
   * @param {Object} options - { rootDirectory: string }
  ...
  async detectFramework(repoPath, options = {}) {
    try {
      ...
      // Try frontend preset detection first (package.json based)
