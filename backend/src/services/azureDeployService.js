...
async function runAciViaPython({ deploymentId, appName, image, socketId, logLines, io }) {
 ...
 */
async function runAzureDeployment({ repoUrl, appName, socketId, io }) {
  const cfg = azureConfig();
  ...
  try {
    ...
    const aciResult = await runAciViaPython({ deploymentId, appName: sanitizedName, image: fullImage, socketId, logLines, io });
    ...
    if (io && socketId) {
      io.to(socketId).emit('deploy:done', { deploymentId, url: appUrl });
    }
  } catch (error) {
    const msg = error?.message || String(error);
    emitLog(io, socketId, `[Azure] Deployment failed: ${msg}`);
    if (io && socketId) {
      io.to(socketId).emit('deploy:error', { deploymentId, message: msg });
    }
  } finally {
